    ...
    <div className="h-screen bg-pink-200 flex items-center justify-center relative overflow-hidden">
      {blocks.map((block) => (
        <React.Fragment key={block.id}>
          <Block block={block} onCreateNew={createNewBlock} onDrag={handleDrag} />
          {block.parentId && (
            <Line
...
const Block = ({ block, onCreateNew, onDrag }) => {
  ...
  return (
    ...
    >
      <span>{block.id}</span>
      <button
        className="bg-pink-700 w-10 h-6 text-sm rounded-sm mt-2"
        onClick={(e) => {
          e.stopPropagation();
